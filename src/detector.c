...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh)
{
    ...
    while(1){
        ...
        draw_detections(im, l.w*l.h*l.n, thresh, boxes, probs, names, alphabet, l.classes,&countp, &total_area);
        printf("total area:%d\n",total_area);
...
void run_detector(int argc, char **argv)
{
    ...
    if(0==strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh);
       ...
       /* if (skip != 0)
        {
            filename = skipping_frame(filenmae,skip);
        } 
        */      
        list *options = read_data_cfg(datacfg);
